    ...
    <form method="post" action="{{ route('shop.update') }}" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            @if($activeOwnedShop?->profileImage)
                <div class="mt-2 mb-2">
                    <img src="{{ asset('storage/' . $activeOwnedShop->profileImage) }}" alt="Current Shop Image"
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="name" value="{{ old('name', $activeOwnedShop?->name) }}"
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="phone" value="{{ old('phone', $activeOwnedShop?->phone) }}"
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" rows="2" name="address" placeholder="Enter shop address"
                required>{{ old('address', $activeOwnedShop?->address) }}</textarea>
            @error('address')
                <p class="text-danger small mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <textarea class="form-control" rows="3" name="description" placeholder="Tell us about your shop..."
                required>{{ old('description', $activeOwnedShop?->description) }}</textarea>
            @error('description')
                <p class="text-danger small mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <select class="form-control" name="status">
                <option value="OPEN" {{ old('status', $activeOwnedShop?->status) === 'OPEN' ? 'selected' : '' }}>Open
                </option>
                <option value="CLOSED" {{ old('status', $activeOwnedShop?->status) === 'CLOSED' ? 'selected' : '' }}>
                    Closed</option>
            </select>
            @error('status')
                <p class="text-danger small mt-1">{{ $message }}</p>
